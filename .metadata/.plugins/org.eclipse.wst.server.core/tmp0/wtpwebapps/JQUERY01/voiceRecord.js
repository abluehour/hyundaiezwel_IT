	...
	if(navigator.mediaDevices){
		...
		.then(stream=>{
			...
			mediaRecorder.onstop = e => {
				...
				a.href = audio.src;
				//alert(audio.src);
				//탸그는 download 속성이 있고 파일명을 주면 해당 파일명을 ㅗ다운로드 진행하게 됨(클릭이벤트 발생 시)
				a.download = clipName;
				a.click();		
			}//mediaRecoder 끝
			...
			mediaRecorder.ondataavailable = e =>{ //마이크를 통해서 전달되는 데이터는 이벤트객체의 data 속성에 담겨 전달됨
				chunks.push(e.data);
			}
