...
$(function(){
	$('#showBtn').on('click', function(){
		//서버에서 포즈 인식 진행 후 결과 값 반환 받았다고 가정
		...
		function drawCanvas(result, src){
			...
			//이미지 객체 생성 후 canvas애 추가 (drawImage 메서드 활용)
			...
			posImage.onload = function(){
				...
				$.each(result, function(i){
					if(this.x !=0 || this.y !=0){
						context.storkeStyle=colors[i]; //
						context.strokeRect(this.x * posImage.width,this.y * posImage.height,2,2);
						let text = this.x.toFixed(2) + ", " + this.y.toFixed(2);
						console.log(text);
						context.font = '10px';
						context.storkeText(text,this.x = posImage.width, this.y=posImage.height);
					}
					values += position[i] = '(' + this.x + ',' + this.y+')</br>';	
				})
					$('#resultDivBox').html(values);//태그 안되어 있으므로 text 대신에 html() 사용
			}
